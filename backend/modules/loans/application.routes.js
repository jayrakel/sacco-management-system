...
const { calculateLoanSchedule } = require('./utils'); // Import the shared logic

// GET MY LOAN STATUS (With Centralized Schedule Logic)
router.get('/status', async (req, res) => {
    try {
        // 1. Fetch Loan Data including grace_period_weeks
        const result = await db.query(
            `SELECT id, status, fee_amount, amount_requested, amount_repaid, 
                    purpose, repayment_weeks, total_due, interest_amount, 
                    disbursed_at, grace_period_weeks 
        ...
        loan.grace_period_weeks = parseInt(loan.grace_period_weeks || 0);

        // 3. Calculate Schedule using Shared Utility
        // This handles pre-payments, grace period checks, and weeks passed automatically
        loan.schedule = calculateLoanSchedule(loan);
