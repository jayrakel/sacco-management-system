...
import api from '../api'; 
import { Wallet, LogOut, Bell, Archive, XCircle, AlertCircle, MailOpen, Handshake, Check, X } from 'lucide-react';
...
export default function DashboardHeader({ user, onLogout, title = "SaccoPortal" }) {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                // 1. Notifications
                const notifRes = await api.get('/api/loan/notifications');
        ...
        <>
            ...
            {notifications.unread.length > 0 && (
                <div className="bg-blue-600 text-white px-4 py-3 shadow-md sticky top-0 z-50">
                    <div className="max-w-6xl mx-auto flex items-center gap-2">
                        ...
                        <span className="text-sm font-bold">{notifications.unread.length} New Notification(s)</span>
                    </div>
            ...
            <nav className="bg-white border-b border-slate-200 px-4 sm:px-6 py-4 flex justify-between items-center sticky top-0 z-40">
                ...
                <div className="flex items-center gap-4">
                    ...
                    <div className="text-right hidden sm:block"><p className="text-xs text-slate-400 font-medium uppercase tracking-wider">{user?.role}</p><p className="text-sm font-bold text-slate-700">{user?.name}</p></div>
