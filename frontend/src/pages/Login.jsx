  ...
  const handleLogin = async (e) => {
    ...
    try {
      ...
      if (user.mustChangePassword) {
        navigate("/change-password"); // <--- Redirects here if flag is true
      } else {
        // Normal routing
        const paths = {
          ADMIN: "/admin",
      ...
      <div className="hidden lg:flex w-1/2 bg-slate-900 flex-col justify-center items-center p-12 text-white relative overflow-hidden">
        ...
        <div className="relative z-10 text-center">
          ...
          <p className="text-slate-400 text-xl max-w-md mx-auto leading-relaxed">
            Empowering members with transparent, automated, and secure financial
            growth.
          </p>
        </div>
      </div>
        ...
        <div className="w-full max-w-md">
          <div className="bg-white p-10 rounded-2xl shadow-xl border border-slate-100">
            ...
            <form onSubmit={handleLogin} className="space-y-6">
              ...
              <button
                disabled={loading}
                className="w-full bg-slate-900 hover:bg-emerald-600 text-white font-bold py-4 rounded-lg transition-all duration-200 flex items-center justify-center gap-2 group"
              >
                {loading ? (
                  "Verifying..."
                ) : (
